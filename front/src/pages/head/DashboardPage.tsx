...
export function DashboardPage() {
  ...
  return (
    <HeadDesktopLayout currentPath="/head/dashboard">
      <div className="mx-auto w-full space-y-4 p-3 max-[600px]:space-y-4 max-[600px]:p-2.5 max-[480px]:p-2 sm:space-y-6 sm:p-6 md:p-8">
        <div className="relative overflow-hidden rounded-3xl border border-white/60 bg-white/55 p-4 max-[600px]:rounded-2xl max-[600px]:p-3.5 max-[480px]:p-3 max-[400px]:p-2.5 sm:p-6 md:p-8 shadow-[0_1px_2px_rgba(11,28,54,0.04),0_24px_60px_rgba(11,28,54,0.10)] backdrop-blur-2xl">
          ...
          <div className="relative flex flex-col gap-4 max-[600px]:gap-3 max-[480px]:gap-2.5 sm:gap-5 md:flex-row md:items-center md:justify-between">
            <div className="space-y-1.5 max-[600px]:space-y-1 sm:space-y-2">
              <span className="inline-flex items-center gap-2 rounded-full border border-amber-200/70 bg-amber-50/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-amber-700 backdrop-blur-xl max-[600px]:gap-1.5 max-[600px]:px-2.5 max-[600px]:py-0.5 max-[600px]:text-[10px] max-[600px]:tracking-[0.08em] max-[480px]:px-2 max-[480px]:text-[9px]">
                <span className="h-1.5 w-1.5 rounded-full bg-amber-500" />
                Панель керівника
              </span>
              <h1 className="max-w-[17ch] break-words font-heading text-[clamp(1.52rem,8.4vw,2.3rem)] font-semibold leading-[1.06] tracking-[-0.03em] text-slate-900 max-[480px]:text-[clamp(1.25rem,7.3vw,1.65rem)] max-[480px]:leading-[1.08] sm:max-w-none sm:text-4xl md:text-5xl">
                Огляд аудиту {user?.name ? `громади ${user.name}` : "громади"}
              </h1>
              <p className="max-w-xl text-sm leading-relaxed text-slate-500 max-[600px]:text-[13px] max-[600px]:leading-snug max-[480px]:text-[12px] md:text-base">
                Аналітика податкових розбіжностей, фінансових втрат та призначених інспекцій
              </p>
            </div>
            <Button size="lg" className="w-full gap-2 max-[600px]:h-10 max-[600px]:text-base max-[480px]:h-9 max-[480px]:text-sm sm:w-auto" onClick={() => navigate("/head/import")}>
